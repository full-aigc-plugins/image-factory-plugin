...
| `skills/`（8 个） | 5 个 Factory 路由/治理技能 + 3 个原样 Baoyu 图片技能 | 运行时强制或上游技能所有权 |
...
- 只使用 Factory 批量流程时不需要 API Key 或 npm 依赖。
- 直接使用 `baoyu-image-gen` 需要 `bun` 或 `npx`；所选提供商可能需要其文档声明的凭据。`codex-cli` 提供商则复用已登录的 Codex CLI。
