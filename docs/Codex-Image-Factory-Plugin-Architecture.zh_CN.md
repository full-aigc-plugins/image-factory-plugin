...
Codex 对话就是产品界面。Skills 在对话中展示方向选项、简短创作确认卡、准确调用数、
带编号的结果和下一步选择；计划、幂等键、回执与路径保存在磁盘供审计，不要求用户
操作表单或理解内部文件。
...
1.1.0 把"花费"从一段循环变成一次事务，改动这批代码前最需要理解的就是它的形状。

- **每个作业只有一个写者。** 所有会改状态的命令在读取台账之前先对 `<job>.lock`
  取排他咨询锁，因此第二个 `run` 无法判定同一项仍待处理。争用会以
  `job_already_running` 报告，且不产生任何花费。
- **批准是绑定的，不是布尔值。** 一条批准记录计划哈希、轮次与仍然欠下的调用数。
  计划一旦变化——改写 prompt、增减图片、更换参考图、进入新一轮——当前批准失效，
  同时历史保留作为审计线索。
- **调用前先占位。** 批次项在外部调用之前进入 `Attempting` 并递增尝试计数，因此
  中断不会导致对可能已经执行过的工作再发起一次调用。只有没有尝试记录的项才算待处理。
- **回执是唯一真源。** 每件产物的回执是按幂等键原子写入的独立文件；聚合清单由这些
  回执读回并与它们所指的文件核验后派生。
- **未决即停止。** 超时或产物消失会把该项留在 `Unknown` 并终止批次，因为在前一个
  结果未决时继续花费后面的项，正是把一次中断变成两次扣费的方式。
- **恢复正常不花费。** `recover` 只依据磁盘上已有的回执判定被中断的项、重建清单，
  并拒绝猜测：没有证据的项保持 `Unknown`，命令以 `recovery_required` 结束。
- **人工标注是强制的。** 1.1.0 计划要求人工结果标注，因此未标注的批次会停在
  `pending_approval`，不能自行通过。

历史 1.0.0 计划与台账由 `scripts/contract_migrations.py` 在内存中升级。迁移可以收紧
文档——两道人工门禁都变为强制——但绝不能凭空造出未观察到的批准；已经带有批准证据的
1.0.0 作业会被拒绝迁移而不是被迁移。

## 10. 持续集成

`.github/workflows/ci.yml` 在 Ubuntu、macOS、Windows 上以 Python 3.11 与 3.13 运行
完整套件。每个作业都会编译全部脚本与测试、运行整个离线套件、校验发行结构并检查空白。
任何作业都刻意不安装任何依赖：一旦安装了包，它就不再是"插件不需要任何依赖"的证据。

## 11. 演进

本文件只描述已实现的图片内核和提示词检索层。工作台界面、父项目状态与非图片
...
- **图片派生产物。** Recipe、fixture、回执与评分继续围绕图片塑形；非图片媒体由其所属产品负责。
