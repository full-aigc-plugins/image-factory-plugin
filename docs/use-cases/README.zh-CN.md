...
- **Image Factory 后续**：属于图片生产边界，但尚未获得当前版本运行证据。
...
| [图片编辑与审阅](edit-review.zh-CN.md) | 8 | 局部修改、文字替换、轮次、批准与恢复 |
| **合计** | **68** | 覆盖 Image Factory 图片生产、编辑和审阅场景 |
...
主体、用途、文案、受众和参考图说清楚即可。先使用 `prompt-search` 查找带来源的模板，
再由 Codex 改写为批次计划；运行前使用 `quote` 查看调用数量并明确批准。
