...
用户只需说目标。系统先给出最多三个视觉方向，每个方向包含可见预览、适用理由、
大致构图和风格标签。用户点一个方向，Creative Director 再把模板改写为用户的
主体、场景和文字。这样比先展示长 prompt 更容易做决定。
...
界面默认隐藏 prompt、JSON、幂等键、模型和文件路径。用户只看到目标、方向、
条件、调用数量、进度、图片和“采用/调整”。高级信息放在折叠面板，供需要审计
的用户查看。

## 4. 统一分类体系

一级创作类型保持少而稳定：单图、系列视觉、绘本/漫画、社交内容、电商/产品、
海报/活动、信息图/科普、角色/IP、UI/网页、游戏素材、视频封面、本地故事视频。
...
flowchart LR
  ...
  L[许可证与来源] --> N
  N --> D[Creative Director]
  I[用户目标与选择] --> D
  D --> P[Image Batch Plan]
...
本地视频由已批准图片、字幕、旁白和 FFmpeg 合成，不宣称原生文生视频能力。
...
- 插件目前没有自定义 Codex 原生面板。完整傻瓜化界面需要一个按需启动、只监听
  `127.0.0.1` 的本地 Creative Studio。
- 视频首版是图片编排、推拉、转场、字幕与本地旁白合成，不生成连续角色动作。
