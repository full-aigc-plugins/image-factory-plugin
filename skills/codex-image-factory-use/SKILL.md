...
description: >
  Use when the user wants to produce a batch of images, run an image
  production round, or continue an image batch that already exists. Routes to
  `codex-image-factory-run` for a batch that has not been generated yet, to
  `codex-image-factory-judge` for a batch that has results to evaluate, and to
  `codex-image-factory-recover` for a batch whose state is unclear or that was
  interrupted. This skill only picks the entry point and never performs the work
  itself. For a single ad-hoc image with no batch plan, no skill in this plugin
  applies: ask Codex for the image directly.
---
...
1. Establish whether a batch plan already exists. Look for a plan file the user
   named, or for a job ledger left by an earlier round.
2. Classify the request into exactly one of three situations:
...
3. State which situation you detected and why, then delegate and stop.
