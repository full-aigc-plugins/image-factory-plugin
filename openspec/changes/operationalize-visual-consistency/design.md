...
0.8.0 的多哈希策略为显式 opt-in：计划声明 aHash/dHash/pHash 各自距离阈值和最少命中数，scores 保存策略版本、算法版本与逐对距离；旧单 aHash 阈值继续独立执行，二者互斥。阈值示例不是生产校准结果。校准默认最少 30 个标签，输出 Wilson 95% 区间与模型/供应商/画风/镜头分层；不足样本不产生候选阈值。评审器漂移只与显式批准基线比较并触发人工复核。

审片页的 anchor 映射仅能引用本批已核验回执；标注下载为 JSON 草稿，不暗改 scores 或台账。runtime matrix 初始 11 个案例均为 `NOT_RUN`，新增模型组合也从 `NOT_RUN` 开始；只有带起止时间、`live_runtime` 来源和证据引用的记录才能进入 `PASS`/`FAIL`。这是记录完整性约束，不自动验证外部证据真实性。390px 静态 CSS 检查已运行，浏览器真实视口验收因本地页面访问策略受阻而继续未完成。
